...
import com.minagri.stats.vaadin.component.combobox.LCMMultiSelectComboBox;
...
public interface IsMultiSelectComboBoxFluent<FLUENT extends LCMMultiSelectComboBox<VALUECLASS>, VALUECLASS> extends HasTranslation {
    ...
    default FLUENT withTextRenderer(Function<VALUECLASS, String> textProvider) {
        getFluent().setRenderer(new TextRenderer<>(textProvider::apply));
        return getFluent();
    }

    default FLUENT withLocalizedTextRenderer(Function<VALUECLASS, String> textProviderNl, Function<VALUECLASS, String> textProviderFr) {
        getFluent().setRenderer(new ComponentRenderer<>(item -> new LCMSpan()
                .withStyle("white-space", "pre-wrap")
                .setLocalizedText(textProviderNl.apply(item), textProviderFr.apply(item))));
    ...
    default FLUENT withSmallTheme() {
        getFluent().addThemeVariants(MultiSelectComboBoxVariant.LUMO_SMALL);
        return getFluent();
    }
